...
function modular_computers.motherboard.get_inventory_tier_from_inventory(
    ...
    local itemstack = modular_computers.motherboard.get_itemstack_from_id(id)
    local meta = itemstack:get_meta()
    return meta:get_string("tier")
end

function modular_computers.motherboard.get_inventory_tier_from_id(id)
    local itemstack = modular_computers.motherboard.get_itemstack_from_id(id)
    local meta = itemstack:get_meta()
    return meta:get_string("tier")
end
...
function modular_computers.register_motherboard(item_name, item_description,
    ...
    minetest.register_craftitem("modular_computers:motherboard_" .. item_name, {
        ...
        on_secondary_use = function(itemstack, player, pointed_thing)
            ...
            if meta:get_string("tier") == "" then
                meta:set_string("tier", tier_number)
            else
                tier_number = meta:get_string("tier")
            end
            ...
            if modular_computers.motherboard.check_exists(id) == false then
                local inv = minetest.create_detached_inventory(
                                "modular_computers:motherboard_inventory_" .. id,
                                modular_computers.motherboards[item_name]
                                    .detached_inventory)
